...
m4_ifelse(TOOLSET, llvm, `
	...
	m4_include(__details/sanity|llvm.m4)
	...
	m4_ifdef(`__USE_LLVM_AS', `
		m4_include(__details/toolset|llvm_as.m4)
	', `
		m4_ifdef(`EMSCRIPTEN', `
...
__HACK_SPACE`'command=__AR_EXEC -rcs $out $in
	...
	m4_undefine(`__AR_EXEC')
', `
	ERROR(invalid `TOOLSET' value: TOOLSET)
')

m4_ifelse(LD, lld, `
	m4_define(__LD_EXEC, ld.lld)
	m4_define(__LD_TYPE, `llvm')
', LD, wasm-lld, `
...
', LD, gold, `
	...
	m4_define(__LD_TYPE, `gnu')
', LD, mold, `
	m4_define(__LD_EXEC, mold)
	m4_define(__LD_TYPE, `gnu')
', LD, bfd, `
	m4_ifdef(`MINGW', `
