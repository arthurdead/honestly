...
m4_ifdef(`EMSCRIPTEN', `
	...
	m4_ifelse(ARCHITECTURE, wasm-32, `
		m4_define(`__VALID_ARCHITECTURE', `')
	')
	m4_ifdef(`__VALID_ARCHITECTURE', `m4_undefine(`__VALID_ARCHITECTURE')', `ERROR(`EMSCRIPTEN' requires `ARCHITECTURE'=wasm-32 or `ARCHITECTURE'=wasm-64)')
	...
	m4_ifelse(LIBC, cloudlibc, `
		...
		')
	')
	m4_ifdef(`__VALID_LIBC', `m4_undefine(`__VALID_LIBC')', `ERROR(`EMSCRIPTEN' requires `LIBC'=musl or `LIBC'=cloudlibc (with `WASI'))')
...
m4_ifdef(`MINGW', `
	...
	m4_ifelse(LD, bfd, `', `
		m4_define(__VALID_LD, `')
	')
	m4_ifdef(`__VALID_LD', `m4_undefine(`__VALID_LD')', `ERROR(`MINGW' requires `LD'=ld or `LD'=bfd)')
', `
	m4_ifelse(LIBC, crt, `
